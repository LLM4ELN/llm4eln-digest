"""Tools for ELN data processing and SPARQL query handling."""
...
class GetCurrentTimeTool(BaseTool):
    ...
    description: str = (
        "Get the current time and date. Useful for timestamping ELN entries or checking when data was processed."
    )
...
class CalculateMetadataStatsInput(BaseModel):
    """Input for calculating metadata statistics."""

    entry_count: int = Field(description="Number of ELN entries")
    has_metadata: int = Field(description="Number of entries with metadata")


class CalculateMetadataStatsTool(BaseTool):
    """Tool for calculating ELN metadata statistics."""

    name: str = "calculate_metadata_stats"
    description: str = (
        "Calculate statistics about ELN metadata coverage. Useful for analyzing how well ELN entries are documented."
    )
    args_schema: type[BaseModel] = CalculateMetadataStatsInput

    def _run(self, entry_count: int, has_metadata: int) -> dict[str, Any]:
        """Calculate metadata statistics.

        Args:
            entry_count: Total number of ELN entries
            has_metadata: Number of entries with metadata
        ...
        coverage = (has_metadata / entry_count) * 100
        missing = entry_count - has_metadata

        return {
            "total_entries": entry_count,
            "entries_with_metadata": has_metadata,
            "coverage_percentage": round(coverage, 2),
            "missing_metadata": missing,
        }

    async def _arun(self, entry_count: int, has_metadata: int) -> dict[str, Any]:
        """Async version of _run."""
        return self._run(entry_count=entry_count, has_metadata=has_metadata)
...
calculate_metadata_stats_tool = CalculateMetadataStatsTool()
update_preview_tool = UpdatePreviewTool()

# List of all available tools
AVAILABLE_TOOLS = [get_current_time_tool, calculate_metadata_stats_tool, update_preview_tool]
