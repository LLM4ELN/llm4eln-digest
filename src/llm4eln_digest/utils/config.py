...
def _find_config_path() -> Path:
    """Locate ``config.yml`` by walking up from the package directory.

    The search order is:
    1. ``LLM4ELN_CONFIG_PATH`` environment variable (if set).
    2. Walk upward from the directory containing this module until
       ``config.yml`` is found or the filesystem root is reached.

    Raises:
        FileNotFoundError: If the file cannot be found.
    """
    env_path = os.getenv("LLM4ELN_CONFIG_PATH")
    if env_path:
        p = Path(env_path)
        if p.is_file():
            return p
        msg = f"LLM4ELN_CONFIG_PATH points to a non-existent file: {env_path}"
        raise FileNotFoundError(msg)

    current = Path(__file__).resolve().parent
    while True:
        candidate = current / "config.yml"
    ...
    msg = "config.yml not found (searched from package directory upward)"
    raise FileNotFoundError(msg)
